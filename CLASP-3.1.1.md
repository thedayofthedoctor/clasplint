## PEP 2606 — CLASP Stage 3.1.1 (Code Logging Annotation Standard Proposal)
...
第十一：每个 Python 文件必须在文件头部包含编码声明，格式为 `# -*- coding: utf-8 -*-` 或等效形式（如 `# coding: utf-8`）。编码声明必须位于文件的第一行或第二行（当第一行是 shebang 时）。

第十二：仅包含非字母字符的物理行（如单独的 `}`、`]`、`)`、`],`、`], [`、`},` 等），豁免逐行注释要求。此豁免仅适用于不含任何英文字母（A-Z、a-z）的行，一旦行内出现任何字母字符，则该行不可享受豁免，必须添加注释。此类纯标点行上不得出现注释；若已有注释，应删除或将注释内容迁移至其对应的前序非纯标点代码行。

第十三：每条注释必须为独立的单行句子，禁止使用连续多行注释块来表述同一内容。若注释内容过长无法在一行内完整表达，应精简措辞至单行，或将内容拆分至不同的代码段落各自独立注释。

第十四：注释必须解释代码意图而非复述代码本身。禁止使用 `Check if ...`、`Check whether ...` 等仅转述代码逻辑而不揭示设计意图的弱注释模式。

第十五：所有注释文本必须使用英文书写，不得出现中文字符或其他非 ASCII 字符。仅允许 shebang（`#!`）、类型注解标记（`# type:`）、noqa 标记（`# noqa`）和编码声明（`# -*- coding: ...`）等特殊标记行使用固定格式。
